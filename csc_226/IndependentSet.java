...
public class IndependentSet{
	...
	static void Findsets(int[][] G, VertexSet best, VertexSet current, VertexSet forbidden, int v){
		int n = G.length;
		...
		if(v == n){
			...
			return;
		} // if current set is bigger the best a copy of it
		
		if((current.getSize()+n-forbidden.getSize()) <= best.getSize()){
			return;
		} //if the size of the current set + the size of possible additional vertices is less than the best then this set can not beat the best so return.
	
		VertexSet forbidden2 = new VertexSet(n);
		forbidden2.copyFrom(forbidden);
		forbidden2.addVertex(v);
		Findsets(G, best, current, forbidden2, v+1);
		
		
		if(!forbidden.isInSet(v)){
			current.addVertex(v);
			for(int k = 0; k < n; k++){
				if(G[v][k] == 1){
					forbidden2.addVertex(k);
				} //if
			} //for
			Findsets(G, best, current, forbidden2, v+1);
