...
public class AVLTree{
	...
	*/
	public TreeNode insert(String s){
		...
		while(true){
				if(s.compareTo(tracker.nodeValue) < 0){
				leri = 1;
			...
			} else if(s.compareTo(tracker.nodeValue) > 0){
				leri = 0;
				if(tracker.rightChild != null){
		...
		if(leri == 1){
			tracker.leftChild = newnode;
		} else {
			tracker.rightChild = newnode;
		}
		tracker = newnode;
		newnode.recomputeHeight();
		/*
		while(tracker != root){
			
			if(tracker == tracker.parent.leftChild){
				if((getTreeHeight(tracker) - getTreeHeight(tracker.parent.rightChild)) == -1){
					if((getTreeHeight(tracker.leftChild) - getTreeHeight(tracker.rightChild)) == 1){
						rotateleft(tracker);
					} else if((getTreeHeight(tracker.leftChild) - getTreeHeight(tracker.rightChild)) == 1){
						rotateright(tracker);
						break;
					}
				} //if loop leftright or leftleft case
				recomputeHeight();
				
			} else if(tracker == tracker.parent.rightChild){
				if((getTreeHeight(tracker) - getTreeHeight(tracker.parent.leftChild)) == 1){
					if((getTreeHeight(tracker.leftChild) - getTreeHeight(tracker.rightChild)) == -1){
						rotateleft(tracker);
					} else {
						rotateright(tracker);
	...
	*/
	public void remove(TreeNode node){
		...
		while(tracker != null){
			if(node.nodeValue.compareTo(tracker.nodeValue) > 0){
				tracker = tracker.leftChild;
				if(tracker == node){
					if(tracker.leftChild == null && tracker.rightChild == null){
						tracker = null;
					} else if(tracker.leftChild != null && tracker.rightChild != null){
						if(tracker.parent.leftChild == tracker){
							tracker.leftChild.parent = tracker.parent;
							tracker.parent.rightChild = tracker.leftChild;
							tracker.rightChild.parent = tracker.leftChild;
							tracker.leftChild.rightChild = tracker.rightChild;
							tracker = null;
						} else if(tracker.parent.rightChild == tracker){
							tracker.rightChild.parent = tracker.parent;
							tracker.parent.rightChild = tracker.rightChild;
							tracker.leftChild.parent = tracker.rightChild;
							tracker.rightChild.leftChild = tracker.leftChild;
							tracker = null;
						}
					} else if(tracker.leftChild == null && tracker.rightChild != null){
						tracker.parent.rightChild = tracker.rightChild;
						...
						tracker.parent.leftChild = tracker.leftChild;
						tracker.leftChild.parent = tracker.parent;
						tracker = null;
					}
				}
