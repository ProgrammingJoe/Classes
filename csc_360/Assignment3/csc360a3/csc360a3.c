...
/*
void append_file(char * mp, FILE * file, int offset, int blocksize){
	offset = offset*blocksize;
	int filesize = get_super_info(mp, offset+9 , 4);
	char * the_file = (char *)malloc(sizeof(char) * filesize);
	printf("test\n");

	the_file = memcpy(the_file, mp+offset, filesize);// seg fault here
	printf("test\n");

	fwrite(the_file, filesize, 1, file);
	if((get_super_info(mp, offset, 4)) == -1){
		exit(1);
	} else {
		...
		for (j = 0; j < 8; j ++){ 
			...
			if ((root_entry[0] & 0x02) == 0x02){
				...
				if(strcmp(file_name_bytes, filename) == 0){
					...
					file = fopen(name, "wb");
					fwrite(the_file, filesize, 1, file);
					//if((get_super_info(mp, start, 4)) == -1){
						exit(1);
					//} else {
