...
def main():
    ...
    for vector in usedvectors:
        vectordict[vector.split()[0]] = normalize([float(i) for i in vector.split()[1:]])
    ...
    for comparison in comparisons:
        ...
        if(len(analogies) == 4 and vectordict.has_key(analogies[0]) and vectordict.has_key(analogies[1]) and vectordict.has_key(analogies[2])):
            test = normalize(np.array(vectordict[analogies[1]]) - np.array(vectordict[analogies[0]]) + np.array(vectordict[analogies[2]]))
            fivebest = {}
            ...
            for key, value in vectordict.items():
                ...
                if(len(fivebest) < 5):
                    ...
                    biggest = min(fivebest, key=fivebest.get)
                    if(fivebest[biggest] < result):
                        del fivebest[biggest]
                        fivebest[key] = result
            smallest = max(fivebest, key=fivebest.get)
            if(analogies[3] == smallest):
                closest = closest + 1
